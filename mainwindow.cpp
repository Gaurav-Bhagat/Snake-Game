...
void MainWindow::startGame() {
    ...
    obstacles.clear();
    speed = 200;
    snakeBody.push_back(QPoint(7, 7));  // Start snake at center
...
void MainWindow::togglePause() {
    if (gameRunning) {
        if (isPaused) {
            // Resume the game
            timer->start(speed);  // Adjust the interval to your needs
...
void MainWindow::gameOver() {
    ...
    QPushButton *newGameButton = msgBox.addButton("New Game", QMessageBox::AcceptRole);  //AcceptRole is used to define the role of the button
    QPushButton *quitButton = msgBox.addButton("Quit", QMessageBox::RejectRole);  //same as above

    msgBox.exec(); // Display the message box

    // Check which button was clicked
    if (msgBox.clickedButton() == newGameButton) {
        startGame();  // Start a new game if New Game is pressed
    } else if (msgBox.clickedButton() == quitButton) {
        close();  // Close the game if Quit is pressed
    }
}
...
void MainWindow::paintEvent(QPaintEvent *event) {
    QPixmap pixmap(ui->workArea->size());  // Match the size of the workArea
    //pixmap.fill(Qt::white);  // Fill the background with white

    QPainter painter(&pixmap);  // Use the QPixmap for painting
    ...
    // gradient.setColorAt(0, QColor(20, 50, 100));

    // gradient.setColorAt(1, QColor(131, 111, 255));
    ...
    // QPixmap backgroundImg("background.jpg");

    // painter.drawPixmap(0,0,width,height, backgroundImg);
    ...
    // Set the pixmap to the QLabel (ui->workArea)
